...
IF OBJECT_ID(N'[DDI].[BusinessHoursSchedule]', 'U') IS NULL
...
GO
PRINT N'Creating primary key [PK_BusinessHoursSchedule] on [DDI].[BusinessHoursSchedule]'
GO
IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[DDI].[PK_BusinessHoursSchedule]', 'PK') AND parent_object_id = OBJECT_ID(N'[DDI].[BusinessHoursSchedule]', 'U'))
