...
{
    public class SysDatabaseFilesHelper : SystemMetadataHelper
    {
        public const string SysTableName = "SysDatabaseFiles";
        public const string SqlServerDmvName = "sys.database_files";

        public static List<SysDatabaseFiles> GetExpectedValues()
        {
            SqlHelper sqlHelper = new SqlHelper();
            var expected = sqlHelper.ExecuteQuery(new SqlCommand($@"
            SELECT * 
            FROM {DatabaseName}.{SqlServerDmvName} T 
            WHERE T.name = '{DatabaseFileName}'"));

            List<SysDatabaseFiles> expectedSysDatabaseFiles = new List<SysDatabaseFiles>();

            foreach (var row in expected)
            {
                var columnValue = new SysDatabaseFiles();
                ...
                expectedSysDatabaseFiles.Add(columnValue);
            }

            return expectedSysDatabaseFiles;
        }

        public static List<SysDatabaseFiles> GetActualValues()
        {
            SqlHelper sqlHelper = new SqlHelper();
            ...
            WHERE D.name = '{DatabaseName}'
                AND T.name = '{DatabaseFileName}'"));

            List<SysDatabaseFiles> actualSysDatabaseFiles = new List<SysDatabaseFiles>();
            ...
            foreach (var row in actual)
            {
                ...
                actualSysDatabaseFiles.Add(columnValue);
            }

            return actualSysDatabaseFiles;
        }
