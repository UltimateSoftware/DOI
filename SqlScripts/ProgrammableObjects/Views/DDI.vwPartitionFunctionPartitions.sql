...
CREATE OR ALTER VIEW DDI.vwPartitionFunctionPartitions
AS

/*
	select * from DDI.vwPartitionFunctionPartitions
*/

SELECT	PFI.PartitionFunctionName,
...
BEGIN
	ALTER DATABASE ' + DBFilePath.DatabaseName + '
		...
		(
			...
			SIZE = ' + CAST(SSIS.InitialSize AS NVARCHAR(20)) + ' MB, 
			MAXSIZE = UNLIMITED, 
			FILEGROWTH = ' + CAST(SSFG.FileGrowth AS NVARCHAR(20)) + ' MB
		) 
...
FROM (  SELECT	TOP (2147483647) *
        ...
        FROM DDI.PartitionFunctions PFM
	        CROSS APPLY (   SELECT PFM2.MinValueOfDataType 
                            FROM DDI.PartitionFunctions PFM2 
                            WHERE PFM2.PartitionFunctionName = PFM.PartitionFunctionName) MinInterval)V
        ORDER BY PartitionFunctionName, BoundaryValue)PFI
	CROSS APPLY (	SELECT	d.database_id, d.name AS DatabaseName, SUBSTRING(physical_name, 1, CHARINDEX(d.name + N'.mdf', LOWER(physical_name)) - 1) AS DBFilePath
					FROM DDI.SysDatabaseFiles df
                        INNER JOIN DDI.SysDatabases d ON d.database_id = df.database_id
					WHERE df.physical_name LIKE '%.mdf'
                        AND d.name = PFI.DatabaseName) DBFilePath
	CROSS APPLY (	SELECT SettingValue AS InitialSize
					FROM DDI.SystemSettings
					WHERE SettingName = 'DBFileInitialSizeMB') SSIS
	CROSS APPLY (	SELECT SettingValue AS FileGrowth
					FROM DDI.SystemSettings
					WHERE SettingName = 'DBFileGrowthMB') SSFG
	OUTER APPLY (	SELECT 1 AS DoesPartitionExist, pf.function_id, ps.data_space_id, ps.name
					FROM DDI.SysPartitionFunctions pf
						INNER JOIN DDI.SysPartitionRangeValues prv ON prv.function_id = pf.function_id
                        INNER JOIN DDI.SysPartitionSchemes ps ON ps.function_id = pf.function_id
