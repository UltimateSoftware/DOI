IF OBJECT_ID('[DDI].[fnGetColumnListForTable]') IS NOT NULL
	DROP FUNCTION [DDI].[fnGetColumnListForTable];
...
SET ANSI_NULLS ON
GO
CREATE   FUNCTION [DDI].[fnGetColumnListForTable](
	@SchemaName					SYSNAME, 
	@TableName					SYSNAME, 
	@ListType					VARCHAR(50),
...
/*
	select DDI.[fnGetColumnListForTable]('DBO', 'Pays', 'CREATETABLE', 1, NULL, NULL)
	select DDI.[fnGetColumnListForTable]('DBO', 'Pays', 'insert', 1, 'S', 'D')
*/
...
BEGIN
	...
	FROM DDI.SysTables t
        INNER JOIN DDI.SysDatabases d ON T.database_id = d.database_id
