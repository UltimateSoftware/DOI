IF OBJECT_ID('[DDI].[fnActualIndex_Frag]') IS NOT NULL
	DROP FUNCTION [DDI].[fnActualIndex_Frag];
...
SET ANSI_NULLS ON
GO
CREATE   FUNCTION [DDI].[fnActualIndex_Frag]()   
...
RETURN  (
			...
			FROM DDI.SysIndexPhysicalStats p
                INNER JOIN DDI.SysDatabases d on p.database_id = d.database_id
