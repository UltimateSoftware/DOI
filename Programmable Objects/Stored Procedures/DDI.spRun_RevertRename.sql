IF OBJECT_ID('[DDI].[spRun_RevertRename]') IS NOT NULL
	DROP PROCEDURE [DDI].[spRun_RevertRename];
...
CREATE   PROCEDURE [DDI].[spRun_RevertRename]
	...
	DECLARE Revert_Cur CURSOR LOCAL FAST_FORWARD FOR
		...
		FROM (
				...
				SELECT	DatabaseName,
						...
						'Statistics',
						4
				FROM DDI.vwTables_PrepTables_Statistics
				WHERE SchemaName = @SchemaName
					AND ParentTableName = @TableName
				UNION ALL
				SELECT	DatabaseName,
