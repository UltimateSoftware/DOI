IF OBJECT_ID('[DDI].[spRefreshMetadata_User_Statistics_UpdateData]') IS NOT NULL
	DROP PROCEDURE [DDI].[spRefreshMetadata_User_Statistics_UpdateData];
...
CREATE   PROCEDURE [DDI].[spRefreshMetadata_User_Statistics_UpdateData]
AS
...
FROM DDI.[Statistics] St
    INNER JOIN DDI.SysDatabases d ON d.name = St.DatabaseName
    INNER JOIN DDI.SysSchemas Sc ON St.SchemaName = Sc.name
    INNER JOIN DDI.SysTables t ON t.name = St.TableName
        AND Sc.schema_id = t.schema_id
    LEFT JOIN DDI.SysStats St2 ON St2.database_id = d.database_id
        AND St2.object_id = t.object_id
        AND St2.name = St.StatisticsName
    LEFT JOIN DDI.SysDmDbStatsProperties SP ON St2.database_id = SP.database_id
