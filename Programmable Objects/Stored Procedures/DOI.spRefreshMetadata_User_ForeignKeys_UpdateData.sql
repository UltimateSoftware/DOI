...
--WITH NATIVE_COMPILATION, SCHEMABINDING  --UPDATE...FROM IS NOT SUPPORTED IN NATIVELY COMPILED MODULES.
AS


--BEGIN ATOMIC WITH (LANGUAGE = 'English', TRANSACTION ISOLATION LEVEL = SNAPSHOT)
    UPDATE FKU
    SET FKU.FKName = SUBSTRING(('FK_' + ParentTableName + '_' + ReferencedTableName + '_' + REPLACE(FKU.ParentColumnList_Desired, ',', '_')), 1, 128),
        FKU.ParentColumnList_Actual = FKS.ParentColumnList_Actual,
