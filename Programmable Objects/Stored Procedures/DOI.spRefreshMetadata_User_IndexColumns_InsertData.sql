...
INSERT INTO DOI.IndexColumns(DatabaseName,SchemaName,TableName,IndexName,ColumnName,IsKeyColumn,KeyColumnPosition,IsIncludedColumn,IncludedColumnPosition, Desired, Actual)
SELECT DatabaseName, SchemaName, TableName, IndexName, CL.value, 0, NULL, 0, ROW_NUMBER() OVER(PARTITION BY DatabaseName, SchemaName, TableName ORDER BY TableName), 0, 1
...
INSERT INTO DOI.IndexColumns(DatabaseName,SchemaName,TableName,IndexName,ColumnName,IsKeyColumn,KeyColumnPosition,IsIncludedColumn,IncludedColumnPosition, Desired, Actual)
SELECT DatabaseName, SchemaName, TableName, IndexName, CLD.value, 0, NULL, 0, ROW_NUMBER() OVER(PARTITION BY DatabaseName, SchemaName, TableName ORDER BY TableName), 1, 0
FROM DOI.IndexesColumnStore ICS
...
--UPDATE DESIRED COLUMNS THAT ARE ALSO ACTUAL COLUMNS
UPDATE IC
SET Desired = 1
FROM DOI.IndexColumns IC
