...
IF OBJECT_ID('[DDI].[spRun_RecoverLogRowsFromRollback]') IS NOT NULL
	DROP PROCEDURE [DDI].[spRun_RecoverLogRowsFromRollback];
...
CREATE     PROCEDURE [DDI].[spRun_RecoverLogRowsFromRollback]
	@Log DDI.LogTT READONLY
...
BEGIN ATOMIC WITH (LANGUAGE = 'English', TRANSACTION ISOLATION LEVEL = SNAPSHOT)
    INSERT INTO DDI.Log (   DatabaseName,
                            SchemaName ,
							TableName ,
							IndexName ,
							PartitionNumber ,
