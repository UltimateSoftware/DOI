...
IF OBJECT_ID('DDI.trIndexesRowStore_IndexValidations') IS NOT NULL
DROP TRIGGER DDI.trIndexesRowStore_IndexValidations
GO

IF OBJECT_ID('[DDI].[spIndexValidations]') IS NOT NULL
	DROP PROCEDURE [DDI].[spIndexValidations];
...
CREATE   PROCEDURE [DDI].[spIndexValidations]
...
BEGIN ATOMIC WITH (LANGUAGE = 'English', TRANSACTION ISOLATION LEVEL = SNAPSHOT)
    ...
    FROM DDI.IndexesRowStore IRS
        INNER JOIN DDI.IndexColumns IC ON IC.DatabaseName = IRS.DatabaseName
    ...
    SELECT @ObjectList += IRS.IndexName + ','
    FROM DDI.IndexesRowStore IRS
        INNER JOIN DDI.SysDatabases d ON IRS.DatabaseName = d.name
	    ...
	    INNER JOIN (SELECT name AS ExistingStorage, data_space_id, type_desc AS ExistingStorageType
				    FROM DDI.SysDataSpaces) ExistingDS 
    ...
    FROM DDI.Tables T
	    INNER JOIN (SELECT	SchemaName , 
				    ...
				    SELECT	SchemaName , 
						    ...
						    PartitionColumn_Desired, 
						    Storage_Desired
				    FROM DDI.IndexesColumnStore) AllIdx
		    ON AllIdx.SchemaName = T.SchemaName
			    AND AllIdx.TableName = T.TableName
	    LEFT JOIN DDI.SysPartitionSchemes ps ON AllIdx.Storage_Desired = ps.name
    ...
    FROM (	SELECT SchemaName, TableName, IndexName, OptionDataCompression_Desired
	    ...
	    INNER JOIN (SELECT SchemaName, TableName, IndexName, PartitionNumber, OptionDataCompression
				    FROM DDI.IndexRowStorePartitions IRS
				    UNION ALL
				    SELECT SchemaName, TableName, IndexName, PartitionNumber, OptionDataCompression
				    FROM DDI.IndexColumnStorePartitions ICS) AllIP 
    ...
    FROM DDI.[Statistics] STM
        INNER JOIN DDI.Tables T ON T.SchemaName = STM.SchemaName
    ...
    FROM DDI.[Statistics] STM
        INNER JOIN DDI.IndexesRowStore IRS ON IRS.SchemaName = STM.SchemaName
    ...
    FROM  (	SELECT DatabaseName, SchemaName, TableName, KeyColumnList_Desired
		    ...
		    FROM DDI.IndexesColumnStore) AllIdx
        INNER JOIN DDI.IndexColumns IC ON IC.DatabaseName = AllIdx.DatabaseName
            AND IC.SchemaName = AllIdx.SchemaName
            AND IC.TableName = AllIdx.TableName
    WHERE NOT EXISTS(	SELECT 'True' 
					    FROM DDI.SysSchemas s 
    ...
    WHERE T.IntendToPartition = 1
	    AND NOT EXISTS (SELECT 'True' 
					    FROM DDI.SysColumns c 
                            INNER JOIN DDI.SysTables TS ON TS.database_id = c.database_id
                                AND TS.object_id = c.object_id
                            INNER JOIN DDI.SysDatabases d ON d.database_id = TS.database_id
